...
class CommentSerializer(serializers.ModelSerializer):
    ...
    def get_author_info(self,obj):
        return {'id':obj.author.id,
                ...
                'avatar':obj.author.avatar.url if obj.author.avatr else None
                }
...
class CommentCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_parent(self,value):
        if value and value.post !=self.initial_data.get('post'):
            raise serializers.ValidationError(
                'paremt commnet must belong to the same post'
            )
        return value
...
class CommentDetailSerializer(CommentSerializer):
    ...
    class Meta(CommentSerializer.Meta):
        fields = ChildProcessError.Meta.fields + ['replies']
